   ...
   for (i = 0; i < ringCurves; i++)
   {
      ...
      for (j = 0; j <= ringEdges; j++)
      {
	    ...
	    yCenter = sin(((i + k) % ringCurves + 0.5) * piValue / ringCurves) * yRadius;
            torusY = (ySize + yRadius * cos(((i + k) % ringCurves + 0.5) * piValue / ringCurves)) * sin((j % ringEdges) * piValue / ringEdges);
            torusX = (xSize + xRadius * cos(((i + k) % ringCurves + 0.5) * piValue / ringCurves)) * cos((j % ringEdges) * piValue / ringEdges);
            torusZ = zSize * sin(((i + k) % ringCurves + 0.5) * piValue / ringCurves);
            glVertex3f(torusX, torusY, torusZ);
            glNormal3d(torusX - xCenter, torusY - yCenter, torusZ);
         }
      }
      glEnd();
   }
...
void display()
{
   ...
   glColor3f (1, 0.65, 0);
   torus(80, 80, 2*PI , 1.0,1.0,0.2, 0.05,0.05 , 0,0,0 , 2,2,1 , 0);
   glColor3f (1, 0, 0);
...
int main(int argc,char* argv[])
{
  //  Initialize GLUT and process user parameters
   ...
   //  Create the window
   glutCreateWindow("Peter Huynh - Homework 5: Lighting");
