 ...
 *  Used Example 9 for Perspective
 *
 *  Key bindings:
 ...
 *
 *  x      Toggle between First Person Mode
...
int viewMode=0;        //  Projection Mode
int mode=0;            //  What to display
int fov=55;            //  Field of view (for perspective)
double asp=1;          //  Aspect ratio
double dim = 2.5;      //  Dimension of viewing box
...
void display()
{
   ...
   if (firstPerson)
   {
      ...
      cameraX = +2*dim*Sin(th)*Cos(ph);//+2*dim*Sin(fpRotation);
	  cameraY = +2*dim        *Sin(ph);
      cameraZ = -2*dim*Cos(th)*Cos(ph); //-2*dim*Cos(fpRotation);
   ...
   Print("FP=%s  Angle=%d,%d  Dim=%.1f FOV=%d Projection=%s",firstPerson?"ON":"OFF",th,ph,dim,fov,viewMode?"Perpective":"Orthogonal");
   //  Render the scene
   glFlush();
...
void key(unsigned char ch,int x,int y)
{
   ...
   //  Switch display mode
   else if (ch == 'q' || ch == 'Q') viewMode = 1-viewMode;
   //  Change field of view angle
   else if ((ch == 'r'|| ch == 'R') && ch<179) fov++;
   else if ((ch == 'f'|| ch == 'F') && ch>1) fov--;
   else if (ch == 'x'|| ch == 'X') firstPerson = 1 - firstPerson;
   //  Switch display mode
   ...
   else
   {
      ...
      else if ((ch == 's' || ch == 'S') && dim < 3.00) dim += 0.05;
...
int main(int argc,char* argv[])
{
  //  Initialize GLUT and process user parameters
   ...
   //  Create the window
   glutCreateWindow("Peter Huynh - Homework 4: Projections");
